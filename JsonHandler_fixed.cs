...
namespace Shared.Services
{
    public class JsonHandler
    {
        ...
        public Dictionary<string, object> EnsureTodayReportExists()
        {
            ...
            // If today's report does not exist, fall back to the latest available report instead of creating an empty one
            var latestFilepath = GetLatestReportFilepath();
            if (!string.IsNullOrWhiteSpace(latestFilepath) && !string.Equals(latestFilepath, filepath, StringComparison.OrdinalIgnoreCase))
            {
                _logger.LogInformation("Today's report not found. Loading latest available report instead: {LatestFile}", latestFilepath);
                var latestData = ReadJson(latestFilepath);
                if (latestData != null)
                {
                    return latestData;
                }
