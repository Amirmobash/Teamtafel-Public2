    ...
    public class MainController
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                // - If today's report file exists -> load it.
                // - Otherwise -> load the most recent existing report in Reports folder.
                // - Only if no reports exist at all (first run) -> create today's report.
                Dictionary<string, object>? reportData = null;

                var todayFilepath = _jsonHandler.GetTodayFilepath();
                if (File.Exists(todayFilepath))
                {
                    _logger.LogInformation("Loading today's report file: {File}", todayFilepath);
                    reportData = _jsonHandler.ReadJson(todayFilepath);
                }
                else
                {
                    var mostRecentPath = GetMostRecentReportFilePath();
                    if (!string.IsNullOrWhiteSpace(mostRecentPath) && File.Exists(mostRecentPath))
                    {
                        _logger.LogInformation("Today's report not found. Loading most recent report file: {File}", mostRecentPath);
                        reportData = _jsonHandler.ReadJson(mostRecentPath);
                    }
                }

                if (reportData == null)
                {
                    _logger.LogInformation("No existing reports found. Creating today's report.");
                    reportData = _jsonHandler.EnsureTodayReportExists();
                }

                if (reportData == null)
                {
                    _logger.LogError("Failed to load or create any report");
                    return;
                }

                // IMPORTANT: Reset in-memory state before applying the loaded report.
                // This prevents old state from remaining if a section is missing in the JSON.
                ...
                _logger.LogInformation(
                    "Successfully loaded report data with {EmployeeCount} employees and {ManagerCount} managers",
                    employeeCount, managerCount);

                _logger.LogInformation("Report data keys: {Keys}", string.Join(", ", reportData.Keys));

                // Load settings (merge onto defaults that ResetStateBeforeLoad set)
                if (reportData.ContainsKey("settings"))
                {
                    ...
                    if (settingsData != null)
                    {
                        ...
                        if (Settings.TryGetValue("selected_display_group_id", out var savedGroupObj))
                        {
                            var savedGroupId = savedGroupObj?.ToString();
                            if (!string.IsNullOrWhiteSpace(savedGroupId))
                            {
                ...
                // Ensure shift manager capacity matches settings (after settings were loaded)
                if (Settings.TryGetValue("shift_capacity", out var capObj) && capObj is int capacity)
                {
                    var timeSinceCapacityChange = DateTime.Now - _lastCapacityChange;
                    if (timeSinceCapacityChange.TotalSeconds < 60.0)
                    {
                        _logger.LogInformation(
                            "Preserving user's capacity change ({UserCapacity}) over loaded capacity ({LoadedCapacity})",
                        ...
                        Settings["night_capacity"] = ShiftManager.Capacity;
                    }
                    else
                    {
                ...
                if (reportData.TryGetValue("employees", out var employeesData))
                {
                ...
                if (reportData.TryGetValue("managers", out var managersData))
                {
                ...
                // Load shifts
                if (reportData.TryGetValue("shifts", out var shiftsData))
                    LoadShiftsFromData(shiftsData);

                // Load shift groups
                if (reportData.TryGetValue("shift_groups", out var shiftGroupsData))
                    LoadShiftGroupsFromData(shiftGroupsData);

                // Load absences
                if (reportData.TryGetValue("absences", out var absencesData))
                    LoadAbsencesFromData(absencesData);

                // Load roles
                if (reportData.TryGetValue("roles", out var rolesData))
                    LoadRolesFromData(rolesData);

                // Load tasks
                if (reportData.TryGetValue("tasks", out var tasksData))
                    LoadTasksFromData(tasksData);

                // Load daily task progress
                if (reportData.TryGetValue("daily_task_progress", out var progressData))
                    LoadDailyTaskProgressFromData(progressData);
        ...
        private void ResetStateBeforeLoad()
        {
            // Reset Settings and managers to avoid stale state when loading an older report
            InitializeSettings();
            ...
            // Keep SelectedDisplayGroupId default unless settings overwrite it
            SelectedDisplayGroupId = "default";
        }

        private string? GetMostRecentReportFilePath()
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Reports directory not found: {Dir}", reportsDir);
                    return null;
                }

                var files = Directory.GetFiles(reportsDir, "report_*.json", SearchOption.TopDirectoryOnly);
                if (files.Length == 0)
                {
                    _logger.LogInformation("No report files found in: {Dir}", reportsDir);
                    return null;
                }
                ...
                foreach (var file in files)
                {
                    ...
                    var dateFromName = TryParseReportDateFromFileName(name);

                    if (dateFromName.HasValue)
                    {
                        if (dateFromName.Value > bestDate)
                        {
                            bestDate = dateFromName.Value;
                            bestFile = file;
                        }
                    }
                    else
                    {
                        // If filename parsing fails, fallback to LastWriteTime
                        var write = File.GetLastWriteTime(file);
                        if (bestFile == null || (bestDate == DateTime.MinValue && write > bestWrite))
                        {
                            bestWrite = write;
                ...
                if (!string.IsNullOrWhiteSpace(bestFile))
                    _logger.LogInformation("Most recent report resolved to: {File}", bestFile);

                return bestFile;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error finding most recent report file");
                return null;
            }
        }

        private DateTime? TryParseReportDateFromFileName(string fileName)
        {
            // Expected: report_YYYY-MM-DD.json
            try
            {
                if (string.IsNullOrWhiteSpace(fileName))
                    return null;

                if (!fileName.StartsWith("report_", StringComparison.OrdinalIgnoreCase) ||
                    !fileName.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
                    return null;

                if (fileName.Length < "report_".Length + 10)
                    return null;

                var datePart = fileName.Substring("report_".Length, 10);
